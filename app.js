...
function mkP() {
    return {
        ...
        color: `hsla(${Math.random() * 30 + 335}, 75%, 68%, 0.85)`
    };
}
...
function drawC() {
    ...
    if (cRefill) {
        const needed = 220 - keep.length;
        for (let i = 0; i < needed; i++) keep.push(mkP());
    }
...
window.addEventListener('resize', () => {
    ...
    resizeTimer = requestAnimationFrame(() => {
        if (cActive && cParts.length) {
            const oldW = canvasWidth;
            const oldH = canvasHeight;
            resizeCvs();
            const ratioX = canvasWidth / oldW;
            const ratioY = canvasHeight / oldH;
            for (let p of cParts) {
                p.x *= ratioX;
                p.y *= ratioY;
            }
